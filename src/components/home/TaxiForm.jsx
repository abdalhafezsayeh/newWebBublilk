...
function TaxiForm({setShowSuccess , setReqId}) {
  ...
  const onSubmit = async (values) => {
    ...
    const calculateRide = await calculateRoute(values.departure , values.arrival)
    ...
    <>
      <div className="w-full h-full relative animate__animated animate__fadeIn rounded">
        <form
          ...
          className="w-full h-full p-4 flex flex-col text-text gap-5 bg-white justify-center rounded"
        >
          {/* from & to */}
          ...
          <div className="w-full flex justify-end">
            ...
            >
              {loadingSendRequest ? <Loading width="22px" height="22px" /> : "Send Request"}
            </button>
          </div>
            ...
            {typeRightContent == "luggages" && <div className="w-full flex flex-col gap-5 animate__animated animate__fadeIn">
              ...
              <div className="flex flex-col gap-2">
                ...
                <textarea 
                  {...register("comment")}
                  rows={2}
                  className="w-full border border-secondary rounded-lg min-h-[50px] max-h-[80px] focus:ring-0 focus:outline-none"
                />
              </div>
            </div>}
